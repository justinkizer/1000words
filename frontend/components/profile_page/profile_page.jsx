...
class ProfilePage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {photosChoice: undefined, profileNavSelection: "My Photos",
      followButtonStatus: undefined};
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.params.id !== this.props.params.id) {
      ...
      this.props.fetchUserPhotos(newProps.params.id).then(photos => {
        this.setState({photosChoice: photos.photos, profileNavSelection:
          "My Photos"});
        setTimeout(AOS.refreshHard,500);
      });
    }
  ...
  render() {
    ...
    let followOrEditProfileButton = <FollowButtonContainer
      updateTrigger={this.state.followButtonStatus}
      paramsId={this.props.params.id} ownerId={this.props.userId} />;
    if (this.props.currentUser && (this.props.currentUser.id ===
      parseInt(this.props.userId))) {
      followOrEditProfileButton = <EditProfileButton
        refreshAfterEdit={this.resetAfterAddOrDelete} />;
      completionBasedDescription = this.props.profileDesc ||
        'Thanks for joining! Customize your profile now or start exploring!';
    } else {
      completionBasedDescription = this.props.profileDesc ||
        "This user hasn't written a Profile Description yet, " +
        "but feel free to imagine something really inspiring!";
    }
      ...
      <div>
        <div>
          <img className="cover-photo"
            src={this.props.coverImgUrl}></img>
          <div className="cover-photo-spacer">
        ...
        <PhotoGrid syncFollowButtons={this.syncFollowButtons}
          screenSelected={this.state.profileNavSelection}
          refreshAfterFollow={this.refreshAfterFollow}
          resetAfterDelete={this.resetAfterAddOrDelete}
          currentUser={this.props.currentUser} photos={photosChoice} />
